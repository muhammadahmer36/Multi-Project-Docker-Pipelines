...
import { getCheckedItems, getEmployeeDetailsList, getVisibleApprovedConsent } from '../selectors';
import { setVisibleApprovedConsent, timesheetAction } from '../slice';
import { EmployeeDetail } from '../types';

export default function Approved() {
  const [t] = useTranslation();
  const visible = useSelector(getVisibleApprovedConsent);
  ...
  const handleClose = () => {
    dispatch(setVisibleApprovedConsent(false));
  };

  const handleOk = () => {
    dispatch(timesheetAction({
      actionId: TimesheetActions.Approved,
  ...
  return (
    <TimesheetApproveUnapprovePopup
      t={t}
      headerTitle={t('approveTimesheet')}
      visible={visible}
      handleCancel={handleCancel}
      handleClose={handleClose}
      handleOk={handleOk}
      actionButtonTitle={t('approve')}
      approvedTimesheetContent={t('approvedTimsheetContent')}
    />
  );
}
