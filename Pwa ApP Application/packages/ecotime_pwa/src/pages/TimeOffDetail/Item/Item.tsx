...
import {
  ...
  getSearchConfigurationForTimeOffRequestColor, getSummaryOfItemsDetail,
} from 'pages/TimeOffCalendar/selectors';
import { Chip, SwapHours } from 'components';
...
import { SearchConfiguration, TimeOffRequestItemDetail, reviewStatus as reviewStatusEnum } from 'pages/TimeOffCalendar/types';
...
export default function Item(props: Props) {
  ...
  const { processStatus_Title, reviewStatus_Title, reviewStatus_Code } = summaryOfItemsDetail;
  ...
  const color = getReviewStatusConfiguration
    .find((eachStatusConfiguration: SearchConfiguration):
      boolean => eachStatusConfiguration.search_ReviewStatusCode === reviewStatus_Code.toString())
    ?.displayColor;
  ...
  const displayProcessStatus = () => (
    reviewStatus_Code !== pending && (
      <Grid
        container
        className={`${styles.marginAtLastGrid} ${styles.marginBetweenGrids}`}
      >
        <Grid
          item
          xs={5}
      ...
      {expandedItems[index] && (
        ...
        >
          ...
          >
            ...
            >
              {
                reviewStatus_Code === pending && currentUserMode !== UserRole.Manager
                && (
                  <SwapHours
                    itemIndex={index}
          ...
          <Grid
            container
            onClick={onRowClick}
            className={`${reviewStatus_Code === pending ? styles.marginAtLastGrid : ''} ${styles.marginBetweenGrids}`}
          >
            <Grid
              item
              xs={5}
